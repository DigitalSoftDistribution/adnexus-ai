...
```mermaid
flowchart LR
  prep["Pre-launch: waitlist + MCP package + 10 design-partner agencies"] --> mcp["Phase A: MCP registry + directories + demo threads"]
  mcp --> comm["Phase B: Reddit/Slack/FB value posting + lead magnet"]
  comm --> launch["Phase C: Product Hunt + Show HN + LinkedIn"]
  launch --> content["Always-on: comparison SEO + tutorials + referrals"]
  content --> first100["First 100 paying customers"]
```
