...
| B6 | Sentry/metrics/logging must be verified in the target environment. | API has Sentry and `/metrics` code paths, but DSN/scrape/alert setup must be checked after deploy. | Trigger staging test event, inspect redaction, confirm Prometheus/Grafana or equivalent scrape and alert path. |
