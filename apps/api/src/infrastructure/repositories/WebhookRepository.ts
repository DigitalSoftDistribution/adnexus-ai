import type { IWebhookRepository, WebhookConfig, WebhookDelivery } from '../../domain/repositories/IWebhookRepository';
...
export class WebhookRepository implements IWebhookRepository {
  ...
  async createConfig(config: Omit<WebhookConfig, 'id' | 'createdAt' | 'updatedAt'>): Promise<WebhookConfig> {
    const { rows } = await query<WebhookConfig>(
      `INSERT INTO webhook_configs (workspace_id, name, url, events, secret, active)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [
        config.workspaceId, config.name, config.url,
        JSON.stringify(config.events), config.secret, config.active,
      ],
    );
    return rows[0];
  }
