...
/** Extract a numeric value from a MetaAction or MetaActionValue array */
function findActionValue(
  actions: MetaAction[] | MetaActionValue[] | undefined,
  actionType: string,
): number {
  if (!actions) return 0;
...
/** Extract ROAS from purchase_roas or website_purchase_roas */
function extractRoas(i: MetaInsight): number {
  // Prefer purchase_roas (omni_purchase), then website_purchase_roas
  if (i.purchase_roas) {
    const omni = i.purchase_roas.find(
      (r: MetaPurchaseRoas) => r.action_type === 'omni_purchase',
    );
    if (omni) return parseFloat(omni.value) || 0;
  }
  if (i.website_purchase_roas) {
    const web = i.website_purchase_roas.find(
      (r: MetaPurchaseRoas) => r.action_type === 'omni_purchase',
    );
    if (web) return parseFloat(web.value) || 0;
  }
  return 0;
}

/** Extract CPA from cost_per_action_type */
function extractCpa(i: MetaInsight): number {
  if (!i.cost_per_action_type) return 0;
  const cpa = i.cost_per_action_type.find(
    (a: MetaAction) => a.action_type === 'offsite_conversion' || a.action_type === 'purchase',
  );
...
function mapInsight(i: MetaInsight): UnifiedInsight {
  return {
    ...
    conversions: parseInt(
      findActionValue(i.actions, 'offsite_conversion').toString() || '0',
      10,
    ),
    conversionValue: findActionValue(i.action_values, 'offsite_conversion'),
...
function parseMetaObjectiveForCreate(
  input: UnifiedCampaign['objective'],
): string {
  switch (input) {
    case 'awareness':
      return 'REACH';
...
export class MetaPlatformClient implements PlatformClient {
      ...
      this.client = new MetaApiClient({
        oauth: {
          ...
          redirectUri:
            ...
            'https://api.adnexus.ai/auth/meta/callback',
        },
        ...this.clientConfig,
      });
  ...
  async getCampaign(campaignId: string): Promise<UnifiedCampaign> {
    try {
      ...
      const campaigns = await client.getCampaigns(
        this.account.platformAccountId,
        {},
      );
  ...
  async createCampaign(data: CreateCampaignInput): Promise<UnifiedCampaign> {
    try {
      ...
      const metaData: CreateCampaignData = {
        name: data.name,
        objective: parseMetaObjectiveForCreate(
          data.objective,
        ) as CreateCampaignData['objective'],
  ...
  async getAds(campaignId: string): Promise<UnifiedAd[]> {
    try {
      ...
      const ads = (result.data ?? []).filter(
        (a: MetaAd) => a.campaign_id === campaignId,
      );
  ...
  async updateAd(
    ...
    try {
      ...
      await client.updateAd(
        adId,
        metaData as unknown as { name?: string; status?: MetaAdStatus },
      );
...
function mapMetaErrorCode(metaType: string): 'AUTH_TOKEN_EXPIRED' | 'RATE_LIMIT_HOUR' | 'VALIDATION_INVALID_FORMAT' | 'NOT_FOUND_CAMPAIGN' | 'AUTH_INSUFFICIENT_SCOPE' | 'VALIDATION_POLICY_VIOLATION' | 'UNKNOWN_ERROR' {
  switch (metaType) {
...
export async function connectMetaAccount(
  ...
  try {
    // Use the underlying request method to query /me/adaccounts
    const accountsResult = await (
    ...
    // If /me/adaccounts fails, fall back to adAccountId from auth state
  }
  ...
  const account: AdAccount = {
    ...
    tokenExpiresAt: authState.expiresAt
      ? new Date(authState.expiresAt * 1000).toISOString()
      : undefined,
