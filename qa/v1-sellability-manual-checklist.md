...
| Observability | P1 | Verify `/health`, `/ready`, `/metrics`, Sentry/logging for failed OAuth/Stripe/sync paths. | Health endpoints respond appropriately and critical failures emit searchable logs/errors without secrets. | **PARTIAL** — 2026-06-10 ([SB-3228](https://linear.app/softblaze/issue/SB-3228)): `curl https://adnexus-api.apps.softblaze.net/health` **200** (`status: ok`), `/ready` **200** (`db: ok`, `redis: not_configured`), `/metrics` **200** (Prometheus text incl. `http_request_duration_seconds`). Coolify env on `adnexus-api` + `adnexus-ai`: **no `SENTRY_*` keys** — Sentry inactive at runtime (`initSentry` no-ops without `SENTRY_DSN`; `beforeSend` redaction untested). Prometheus scrape target/alert path not verified from deploy. OAuth/Stripe/sync failure log sweep **MANUAL**. |
